...
"""FastAPI layer over :class:`examples.thinker_talker.airline.database.api.BookingAPI`.

Run directly::

    python -m examples.thinker_talker.airline.database.server

or via ``uvicorn examples.thinker_talker.airline.database.server:app --port 8001``.
...
from examples.thinker_talker.airline.database.db import connect, init_db
from examples.thinker_talker.airline.database.schemas import (
    ActivityEvent,
    AncillariesDiff,
    Booking,
...
if __name__ == "__main__":
    ...
    uvicorn.run("examples.thinker_talker.airline.database.server:app", host=host, port=port, log_level="info")
