...
def get_pnr(pnr: str, api: BookingAPI = Depends(get_api)) -> Booking:
...
def get_activity(pnr: str, api: BookingAPI = Depends(get_api)) -> list[dict]:
...
def ancillaries_diff(
    ...
    api: BookingAPI = Depends(get_api),
) -> AncillariesDiff:
...
def list_alternatives(
    ...
    return api.list_alternatives(origin, destination)
...
def find_by_flight(flight_number: str, api: BookingAPI = Depends(get_api)) -> list[dict]:
...
def get_flight_status(flight_number: str, api: BookingAPI = Depends(get_api)) -> dict:
