...
| [Nemotron Omni Assistant Subagents](src/examples/omni_assistant_subagents/README.md) | Multi-agent **Nemotron Omni** pipeline where specialized agents add audio/video, live-webcam, and user-approved Screen Vision understanding while the voice loop stays responsive. | Recommended for multimodal inputs, giving a richer experience across image, audio, video, webcam, and a shared display. | Cloud, Server (workstation), Single GPU (workstation, DGX Spark) |
