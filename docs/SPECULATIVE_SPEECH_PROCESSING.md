...
Speculative speech processing is enabled through a simple environment variable configuration. When enabled, the system processes both final and stable interim transcripts, generating responses even before the user completes their utterance based on hypothesized transcripts from Riva ASR Service (the interim transcripts). This allows for lower latency responses while maintaining conversation coherence.
