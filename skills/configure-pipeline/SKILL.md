...
- **Local workstation profile hits OOM** -> lower `NIM_KVCACHE_PERCENT` or `NIM_MAX_MODEL_LEN` (when lowering `NIM_KVCACHE_PERCENT` on the hybrid Mamba models, also cap `--max-num-seqs` via `NIM_PASSTHROUGH_ARGS`, since each decode sequence needs one Mamba cache block — otherwise the engine fails CUDA-graph capture at startup). On multi-GPU hosts, choose a NIM profile with matching `tp` and expose that many GPUs.
